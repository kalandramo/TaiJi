...
// 表（TAIJI_USER_PERMISSIONS）缺失时，原实现是「不做用户级判定」——
// 即任何能触发 bot 的用户都能用所有已放行工具。这在 CLI 单用户场景合理，
// 在 serve 多渠道场景是**安全边界消失**。
//
...
func validateServePermissions(in servePermInput) error {
	...
	return fmt.Errorf(
		"权限配置缺失：已装配工具，但未配置用户级权限表——"+
			"任何能触发 bot 的用户都可使用已放行的工具。"+
			"请二选一：(1) 设 TAIJI_USER_PERMISSIONS 按用户管控；"+
