...
| **对接飞书** | 收消息、验签、路由到会话、流式回卡片 | FR-10 | **自建 adapter**（trpc 无渠道层） |
...
证据：WeKnora 两种都实现了——webhook 走 `internal/im/feishu/adapter.go:195` 的 `VerifyCallback`（验签+解密）+ `:313` 的 `ParseCallback`；长连接走 `internal/im/feishu/longconn.go:30` 的 `NewLongConnClient`（内部用 `larksuite/oapi-sdk-go/v3/ws`，`:52` 的 `larkws.NewClient`）。
...
**SDK 选型**：`github.com/larksuite/oapi-sdk-go/v3 v3.9.7`（WeKnora `go.mod:36` 实测采用）。原型**webhook 路径不依赖 SDK**——验签与解密是 60 行标准库代码（AES-256-CBC + token 比对），用 `crypto/aes` 即可，避免为一个哈希比对引入整个 SDK。**长连接路径必须用 SDK**（WebSocket 协议封装）。
...
| happyclaw 的任何代码 | TypeScript，且是模块化单体（`02-设计文档.md` §8.4 已列不借鉴理由） |
...
| 组件 | 路径（拟定） | 复用/自建 | 对应 v1.4 |
|---|---|---|---|
| CLI 入口 | `cmd/taiji/main.go` | 自建 | — |
| Runner 装配 | `internal/bootstrap/runner.go` | 自建（调 trpc） | §5.1 |
| 模型装配 | `internal/bootstrap/model.go` | 自建（调 trpc） | FR-5 |
| MCP 装配 | `internal/bootstrap/mcp.go` | 自建（调 trpc） | FR-4 |
| 飞书 adapter | `internal/channel/feishu/adapter.go` | 自建 | FR-10.1 |
| 飞书验签 | `internal/channel/feishu/verify.go` | 自建（标准库） | FR-10.1 |
| 长连接 | `internal/channel/feishu/longconn.go` | 自建（用 SDK） | FR-10.1 |
| 渠道接口 | `internal/channel/adapter.go` | 自建 | FR-10.1 |
| 触发门禁 | `internal/channel/gate.go` | 自建 | FR-10.4 |
| 会话路由 | `internal/channel/router.go` | 自建 | FR-10.3 |
| 主体解析 | `internal/authz/principal.go` | 自建 | FR-10.2 |
| 上下文降权 | `internal/authz/context.go` | 自建 | FR-10.7 |
| 工具策略 | `internal/authz/toolpolicy.go` | 自建（调 trpc approval） | FR-9.2 |
| 串行化器 | `internal/concurrency/serializer.go` | 自建 | NFR-8 |
| 控制值保护 | `internal/config/guard.go` | 自建 | NFR-9 |
...
原型只实现飞书，但接口按 v1.4 FR-10 的要求"按多渠道路由设计，避免后续接入时重构"。接口形态对齐 WeKnora `internal/im/adapter.go:126`：
...
**刻意省掉 WeKnora 的 `StreamSender` 接口**（`adapter.go:150`，含 `StartStream`/`UpdateStreamContent`/`FinalizeStream`/`EndStream` 四方法）：原型用**简化版流式**——先发一条占位文本消息，收到完整回答后用飞书的**更新消息 API** 替换。代价是看不到逐 token 打字效果，收益是省掉 CardKit 的完整状态机。若验收线 4 要求卡片流式，再补 `StreamSender`（接口位已预留）。
...
原型的路由是 v1.4 FR-10.3 的**简化子集**：

```go
// internal/channel/router.go
// 会话标识格式：{workspaceId}#{localSessionId}
// 话题追加：#thread:{contextId}#root:{rootMessageId}
func ResolveSessionJID(cfg RouteConfig, in *IncomingMessage) string {
    base := cfg.WorkspaceID + "#" + in.ChatID
    if in.ChatType == ChatGroup && cfg.BindingMode == BindingThreadMap && in.ThreadID != "" {
        return base + "#thread:" + in.ThreadID + "#root:" + in.RootMessageID
    }
    return base
}
```

**保留的约束**：会话标识编码了 workspace（`{workspaceId}#...`），这是 v1.4 FR-6.0 的设计——使串行化键派生保持**纯字符串解析、零 IO**（§4.5）。原型**不实现** v1.4 的完整"绑定模式 × 路由模式"矩阵（那是 2×2 组合），只实现 `single_context` 与 `thread_map` 两种最常用的绑定模式。
...
- `internal/channel/adapter.go` 接口
- `internal/channel/feishu/verify.go` 验签 + 解密
- `internal/channel/feishu/adapter.go` 解析 + 回执
