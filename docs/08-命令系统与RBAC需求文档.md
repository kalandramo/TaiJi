...
### 3.4 决策：**不引入 casbin，但把接缝留好**（2026-09-24 用户拍板）

**理由**：

1. **当前规模不需要**——`07-RBAC权限设计.md:64` 已论证：「个位数用户时 RBAC 是过度设计」。
...
| ~~**casbin 过早引入**~~ | 依赖膨胀 + 现有测试作废 | ✅ **已规避**：决策不引入（2026-09-24）；§3.4 留三个触发条件 |
...
## 8. 决策记录（原「待用户确认的问题」——均已有结论）

本需求的两处方向性选择**均已拍板**，无遗留待确认项：

| 决策 | 结论 | 日期 | 影响 |
|---|---|---|---|
| **定位表述** | 同步为两层表述（产品定位 + 实现形态） | 2026-09-24 | `01-需求文档.md` 已改（含 G7/N6），见 §1 |
| **casbin 时机** | **不引入**（保留 `PermissionSource` 接缝） | 2026-09-24 | 零代码改动；§3.4 给出触发条件 |

**casbin 决策的依据**（详见 §3.4）：

- 现有 200 行自研实现含防环、校验、RBAC1 继承，测试完备（含反证）——替换它净收益为负；
- 当前无资源级需求，casbin 的资源级/ABAC 能力处于空转；
- 接缝已就位：满足触发条件时引入 casbin 是**新增一个文件**，不改消费方。

> **这条决策的代价（明说）**：放弃 casbin 意味着**自行维护**角色继承与配置校验逻辑。
> 若将来权限模型复杂到需要 ABAC/资源级，自研的维护成本会超过引入依赖的成本——
> 这正是 §3.4 三个触发条件的作用：把「何时该换」写成可判定的条件，而非凭感觉。
...
| 本文档（`08`） | RBAC **为何不引入 casbin/DB**（含触发条件）、**命令系统要什么** | 需求 |
