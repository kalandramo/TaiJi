...
>
> **抽象来源**：渠道接口形态以 **HappyClaw** 为主参照（`src/im-channel.ts` 的 `IMChannel`、`src/channel-*`、`src/feishu-*`）；**入站解析形态对照 WeKnora**（`internal/im/adapter.go`）。两者的分工见 5.5.2。
>
> **v1.4.2 修订**：本节此前把渠道接口写成单一 `Adapter`（照 WeKnora），与需求文档 FR-10.1 的 `IMChannel` 命名不一致，且无法承载长连接模式。现拆为**连接层 + 入站层**两层，见 5.5.2。

#### 5.5.1 分层

```mermaid
flowchart TB
    subgraph L0["L0 连接层（Channel）"]
        CONN["Channel<br/>Connect / Disconnect<br/>SendMessage / IsConnected"]
    end
    subgraph L1["L1 渠道认证（仅 webhook）"]
        VS["VerifyCallback<br/>验签 + 解密"]
    end
    subgraph L2["L2 主体解析"]
    ...
    MSG["渠道消息"] --> CONN
    CONN -->|"webhook"| VS --> PR
    CONN -->|"长连接<br/>（SDK 回调直出）"| PR
    PR --> RT --> MG --> CAP --> RUN["Agent Run"]
    VS -.->|"失败"| X1["403"]
    PR -.->|"未绑定"| X2["拒绝（fail-closed）"]
    MG -.->|"不满足"| X3["静默丢弃"]
```

**L0 与 L1 的关系**：连接层是**两种模式共有**的（都要建连接、都能出站）；入站层是**webhook 专属**的（长连接模式的入站由 SDK 回调直接产出消息，没有 HTTP 入口，因而不需要验签）。这是两层拆分的根因——见 5.5.2。

#### 5.5.2 渠道接口：连接层 + 入站层

**为什么是两层而非单接口**：飞书支持两种接入形态（webhook 与长连接），两者共有"建连接、发出站消息"，但入站方向不同——webhook 是"被推送"（需要验签的 HTTP 处理器），长连接是"主动拉"（SDK 回调直出消息）。单接口会迫使长连接实现三个永不调用的空方法。

**连接层**（主参照 HappyClaw `IMChannel`，`happyclaw/src/im-channel.ts:224-278`）：

```go
// Channel 每个渠道必须实现（对齐 happyclaw IMChannel）
type Channel interface {
    Platform() Platform

    // Connect 建立连接。
    // webhook 模式：注册路由 + 校验凭据；
    // 长连接模式：启动 WebSocket 并阻塞（对齐 happyclaw feishu.ts:3801 的 connect）。
    Connect(ctx context.Context, opts ConnectOptions) error

    // Disconnect 释放连接。
    // 长连接模式必须真正关闭 socket——SDK 的 Start 不观察 ctx，
    // 取消 ctx 不足以停止（见 5.5.2.1）。
    Disconnect() error

    // SendMessage 出站（对齐 happyclaw IMChannel.sendMessage, im-channel.ts:231-236）
...
**入站层**（对照 WeKnora `Adapter`，`WeKnora/internal/im/adapter.go:126-144`）——**仅 webhook 模式实现**：

```go
// InboundSource 仅 webhook 模式的渠道需要实现。
// 长连接模式的入站由 SDK 回调直接产出 IncomingMessage，不经此接口。
type InboundSource interface {
    // VerifyCallback 验签（对齐 WeKnora feishu/adapter.go:195-235 的两层校验）
...
type IncomingMessage struct {
    ...
    Content   string
    Mentions  []Mention   // @ 判定用此元数据，不用文本匹配
    Meta      *ChannelMessageMeta
}

type ChannelMessageMeta struct {
    Provider          string // 渠道类型
    ChatType          string // p2p | group
    MentionedBot      bool
    NativeContextType string // 仅 "thread" 视为话题（见 5.5.3）
    ContextID         string
    ThreadID          string
    RootID            string
    MessageID         string
...
| **长连接** | 实现（Connect 启 WebSocket） | **不实现**（SDK 回调直出） |

##### 5.5.2.1 长连接的停止语义（实现约束）

长连接的 `Disconnect()` 必须调用 SDK 的 `Close()`，不能只取消 ctx。源码依据（WeKnora `internal/im/feishu/longconn.go:70-76` 的注释原文）：SDK 的 `Start` 阻塞在裸 `select{}`，`Start`/`pingLoop`/`receiveMessageLoop` 都不观察传入的 ctx，因此取消 ctx 会让 socket 存活并继续自动重连；`Client.Close()`（oapi-sdk-go **v3.9.7** 新增）才会真正断开。

**另一处约束**：长连接模式下 **`verificationToken` 与 `encryptKey` 都不需要**（`longconn.go:31-32` 注释原文："Long connection mode does not require verificationToken or encryptKey; those are only used for webhook signature verification and decryption."）。这意味着长连接**没有验签这层安全边界**——信任来自 SDK 与平台的 TLS 通道。部署选型时必须知悉这一点。
...
// ResolveRoute 返回 effectiveJID / agentID / sourceJID
// 对齐 happyclaw/src/channel-inbound-routing.ts:57 的 createChannelInboundRouter
// 三元组形状对齐 happyclaw resolveEffectiveChatJid（im-channel.ts:146-153）
...
**会话标识必须带渠道前缀**（对齐 happyclaw `channel-prefixes.ts:2-11` 的 `CHANNEL_PREFIXES`）：

```go
// 渠道类型 → JID 前缀映射（对齐 CHANNEL_PREFIXES）
var ChannelPrefixes = map[Platform]string{
...
// 从 JID 反解渠道类型（对齐 getChannelFromJid, channel-prefixes.ts:14-19）
func ChannelFromJID(jid string) Platform
```

**为什么必须带前缀**：会话标识若不含渠道前缀，`feishu:xxx` 与 `telegram:xxx` 的同名 chatID 会撞进同一会话，导致跨渠道上下文串扰。

**话题虚拟 JID**（对齐 `channel-native-context.ts:41-47`）：
...
// 飞书话题上下文归一化（对齐 resolveNativeThreadContext, channel-native-context.ts:27-38）
...
**关键**：`botOpenID` 未知时必须**拒绝**——HappyClaw 的文件头注释（`happyclaw/src/feishu-mention-gate.ts:8-10`）记录了历史 bug：旧实现"安全降级=默认放行"导致 `require_mention` 在所有群静默失效（fail-open）；新行为是 fail-closed（`evaluateMentionGate` 在 `:126`，`botOpenID` 缺失即 reject 在 `:146-148`）。

**@ 判定必须用元数据不用文本**：不能用文本匹配判断"bot 被提及"——用户手写 `@name` 会被误判。必须比对平台事件的 mentions 元数据（happyclaw `isBotMentioned` 即比对 `m.id?.open_id === botOpenId`，`feishu-mention-gate.ts:84-85`）。
...
**owner 与 audience 判定**（借鉴 happyclaw `im-audience-policy.ts`）：

```go
// audience 生效值（对齐 effectiveAudienceMode, im-audience-policy.ts:11）
// owner_mentioned 是 owner_only + when_mentioned 的兼容读法
func EffectiveAudienceMode(g Group) AudienceMode

// 发送者是否被 audience 允许（对齐 isSenderAllowedByAudience, im-audience-policy.ts:21-32）
func IsSenderAllowedByAudience(audience AudienceMode, ownerIMID, senderIMID string) bool {
    if audience == AudienceEveryone { return true }
    return senderIMID != "" && ownerIMID != "" && senderIMID == ownerIMID
}

// owner 是否仍有效（对齐 checkOwnerActive, owner-gate.ts:23-30）
// 注意：happyclaw 在无 owner 信息时放行（owner-gate.ts:26），这是为 legacy groups 兼容。
// 本设计要求新渠道 fail-closed —— 无 owner 即拒绝，这是有意的偏离（见 FR-10.5）。
func CheckOwnerActive(owner *Owner) OwnerGateResult
```

**owner 失效必须立即停止响应**（对齐 `owner-gate.ts` 的设计意图，文件头注释 `:5-10`）：owner 被禁用/删除后，bot 不能继续响应到下次重启。happyclaw 在启动时过滤非活跃用户，但 inflight 消息与竞态（重启时机、缓存滞后）仍需**运行期门禁**兜底。
...
// ===== 渠道层（见 5.5.2）=====
// 连接层：两种接入形态（webhook / 长连接）共有
type Channel interface {
    Platform() Platform
    Connect(ctx context.Context, opts ConnectOptions) error
    Disconnect() error   // 长连接必须真正关闭 socket（见 5.5.2.1）
    SendMessage(ctx context.Context, chatID, text string, opts SendOptions) error
    IsConnected() bool
}
...
// 路由（对齐 happyclaw resolveEffectiveChatJid, im-channel.ts:146-153）
type RouteTarget struct{ EffectiveJID, AgentID, SourceJID string }

// 会话串行化（见 4.5）
type Serializer interface {
    Acquire(jid string) (release func(), ok bool)
}
func DeriveSerializationKey(jid string) string
```
...
- **渠道接口形态**（连接层主参照）：`IMChannel` 的 `connect`/`disconnect`/`sendMessage`/`isConnected` 骨架 → 设计文档 **5.5.2**
- **渠道前缀映射**：`CHANNEL_PREFIXES` + `getChannelFromJid`，防跨渠道 chatID 撞车 → 设计文档 **5.5.3**
- **消息元数据契约**：`ChannelMessageMeta` 字段集 → 设计文档 **5.5.2**
- **路由三元组**：`resolveEffectiveChatJid` 的 `{effectiveJid, agentId, sourceJid}` → 设计文档 **5.5.3**
- **owner 与 audience 判定**：`isSenderAllowedByAudience` / `checkOwnerActive` → 设计文档 **5.5.5**
...
- **`StreamSender` / `onFollowUp*` 系列**：卡片流式与会话插话/打断机制，属会话治理范畴，v1 不做（接口位预留）。
- **`owner-gate` 的 fail-open 行为**：happyclaw 在无 owner 信息时放行（`owner-gate.ts:26`）是为 legacy 数据兼容；本设计要求新渠道 fail-closed（**有意偏离**，见 FR-10.5）。
...
### Wave 6：渠道接入（补 FR-10）
- 连接层接口 `Channel` + 入站层接口 `InboundSource`（5.5.2）
- 长连接的停止语义与无验签约束（5.5.2.1）
- 渠道前缀映射 + 消息元数据契约（5.5.2 / 5.5.3）
- 会话路由：绑定模式 × 路由模式 + 话题虚拟 JID（5.5.3）
- 触发门禁 fail-closed（5.5.4）
- 主体解析与 namespace 规范化 + owner/audience 判定（5.5.5）
- 权限降级（5.5.6）
- **验证命令**：`go test ./channel/... -race && go test -tags=integration ./channel/feishu/...`
- **RED→GREEN 要求**：先构造"伪造签名"、"botOpenID 缺失"、"话题串话"、"跨渠道前缀撞车"四个失效场景复现，再实现拦截
- **接线验证**：长连接模式不得实现 `InboundSource`——用编译期断言（`var _ Channel = (*feishuChannel)(nil)`）锁住职责分离
...
| **HappyClaw 渠道连接选项 `IMChannelConnectOpts`** | `happyclaw/src/im-channel.ts:115-222` |
| **HappyClaw 路由三元组 `resolveEffectiveChatJid`** | `happyclaw/src/im-channel.ts:146-153` |
| **HappyClaw 飞书连接 `FeishuConnection`** | `happyclaw/src/feishu.ts:233-241` |
| **HappyClaw 飞书长连接（`lark.WSClient`）** | `happyclaw/src/feishu.ts:1171, 3735, 4015`（注释 `:1150`） |
| **HappyClaw 渠道前缀映射** | `happyclaw/src/channel-prefixes.ts:2-11, 14-19` |
...
| **HappyClaw 话题路由分发** | `happyclaw/src/channel-inbound-routing.ts:57, 82-85, 132, 166` |
| **HappyClaw 话题 agent 创建** | `happyclaw/src/index.ts:19649`（`resolveOrCreateNativeThreadAgent`） |
| **HappyClaw 触发门禁 fail-closed** | `happyclaw/src/feishu-mention-gate.ts:8-10`（历史 bug 注释）、`:126`（`evaluateMentionGate`）、`:146-148`（`botOpenId` 缺失即 reject） |
| **HappyClaw @ 判定用 open_id 元数据** | `happyclaw/src/feishu-mention-gate.ts:84-85`（`isBotMentioned`） |
| **HappyClaw audience 判定** | `happyclaw/src/im-audience-policy.ts:11`（`effectiveAudienceMode`）、`:21-32`（`isSenderAllowedByAudience`） |
| **HappyClaw owner 失效门禁** | `happyclaw/src/owner-gate.ts:5-10`（设计意图注释）、`:23`（`checkOwnerActive`）、`:26`（fail-open 处） |
...
| **WeKnora（次要对照）入站接口 `Adapter`** | `WeKnora/internal/im/adapter.go:126-144` |
| **WeKnora（次要对照）飞书验签** | `WeKnora/internal/im/feishu/adapter.go:195-235` |
| **WeKnora（次要对照）长连接无需 token/encryptKey** | `WeKnora/internal/im/feishu/longconn.go:30-32` |
| **WeKnora（次要对照）长连接 Close 语义** | `WeKnora/internal/im/feishu/longconn.go:63-79` |
