...
01/02 均基于四个参照系项目的**源码直读**：deepseek-harness（dsh）、happyclaw、Tianshu-harness、trpc-agent-go。所有设计断言附 `file:line` 证据索引。03 为原型方案（`taiji-proto`），复用 trpc-agent-go 已有内核，仅权限层与飞书 adapter 自建。
